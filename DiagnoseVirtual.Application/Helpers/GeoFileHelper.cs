...
namespace DiagnoseVirtual.Application.Helpers
{
    public static class GeoFileHelper
    {
        ...
        public static List<Geometria> ReadKml(IFormFile file)
        {
        ...
        public static List<Geometria> ReadWkt(IFormFile file)
        {
            var factory = NTSGeometries.Geometry.DefaultFactory;
            ...
            var NTSPolygons = polygons
                .Select(g => (NTSGeometries.Polygon)factory.CreatePolygon(g.Coordinates.Select(c => new Coordinate(c.X, c.Y)).ToArray())).ToList();
            var geometrias = NTSPolygons.Select(p => (NTSGeometries.Geometry)factory.CreateGeometry(p))
                .Select(g => new Geometria { Geometry = g }).ToList();
            return geometrias;
        }

        public static List<Geometria> ReadShapefile(IFormFile file, string uploads)
        {
            ...
            using (var reader = new ShapeDataReader(filePath))
            {
                ...
                var NTSPolygons = polygons
                    .Select(g => (NTSGeometries.Polygon)factory.CreatePolygon(g.Coordinates.Select(c => new Coordinate(c.X, c.Y)).ToArray())).ToList();
                geometrias = NTSPolygons.Select(p => (NTSGeometries.Geometry)factory.CreateGeometry(p))
                    .Select(g => new Geometria { Geometry = g }).ToList();
            }
